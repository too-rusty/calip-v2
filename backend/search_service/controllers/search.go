...
func Search(w http.ResponseWriter, r *http.Request) {
	...
	var cc []schema.Cc
	err = database.Model(&schema.Cc{}).Where("draft = ?", false).Scopes(db.OrderByCreatedDate(r), db.Paginate(r)).Find(&cc).Error
	if err != nil {
		log.Println(err)
	}
	var tot int64
	err = database.Model(&schema.Cc{}).Where("draft = ?", false).Count(&tot).Error
